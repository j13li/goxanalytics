...
class Secret:
    ...
    def decrypt(self, password):
        ...
        aes = AES.new(crypt_key, AES.MODE_CBC, crypt_ini)
        try:
...
class History(BaseObject):
    ...
    def slot_fullhistory(self, dummy_sender, data):
        ...
        count_added = 0
        if self.gox.dbcon:
            cur = try_get_cursor(self.gox.dbcon, self.gox.config)

        for trade in history:
            date = int(trade["date"])
            price = price2float(trade["price_int"])
            volume = vol2float(trade["amount_int"])
            tid = int(trade["tid"])
            typ = trade["trade_type"]
            ...
            if self.gox.dbcon:
                cur.execute("INSERT IGNORE INTO Trades VALUES(%s, %s, %s, %s, %s)", (tid, date, price, volume, typ))

        if self.gox.dbcon:
            self.gox.dbcon.commit()
            cur.close()
...
class Gox(BaseObject):
    ...
    def __init__(self, secret, config, dbcon):
    ...
    def _on_op_result(self, msg):
        ...
        if reqid == "idkey":
            ...
            parts = reqid.split(":")
            typ = parts[1]
            price = price2float(parts[2])
            volume = vol2float(parts[3])
            oid = result
    ...
    def _on_op_private_ticker(self, msg):
        ...
        if msg["item"] != self.curr_base:
            return
        ask = price2float(msg["sell"]["value_int"])
        bid = price2float(msg["buy"]["value_int"])
    ...
    def _on_op_private_depth(self, msg):
        ...
        if msg["item"] != self.curr_base:
            return
        typ = msg["type_str"]
        price = price2float(msg["price_int"])
        volume = vol2float(msg["volume_int"])
        timestamp = int(msg["now"])
        total_volume = vol2float(msg["total_volume_int"])
    ...
    def _on_op_private_trade(self, msg):
        ...
        price = price2float(msg["trade"]["price_int"])
        volume = vol2float(msg["trade"]["amount_int"])
        typ = msg["trade"]["trade_type"]
        tid = int(msg["trade"]["tid"])
        ...
        if self.dbcon:
            cur = try_get_cursor(self.dbcon, self.config)
            cur.execute("INSERT IGNORE INTO Trades VALUES(%s, %s, %s, %s, %s)", (tid, date, price, volume, typ))
        ...
        p = { "key": "Price", "values": [] }
        p["values"] = [[date, price]]
        d = { "key": "Price", "values": [p] }
        self.send_client_message(d)
    ...
    def _on_op_private_user_order(self, msg):
        ...
        if "status" in order:
            ...
            if order["currency"] == self.curr_quote and order["item"] == self.curr_base:
                volume = vol2float(order["amount"]["value_int"])
                typ = order["type"]
                status = order["status"]
                if "price" in order:
                    # these are limit orders (new or updated)
                    price = price2float(order["price"]["value_int"])
                else:
...
class OrderBook(BaseObject):
    ...
    def __init__(self, gox):
        ...
        self.timer_calc = Timer(2)
        self.timer_calc.connect(self.slot_calc)
    ...
    def slot_calc(self, dummy_sender, data):
        if self.got_full_depth:
            self.calc_total_depth()
    ...
    def slot_depth(self, dummy_sender, data):
        ...
        # Data retrival disabled since the death of MtGox...
        #self.log_order(price, _voldiff, typ)
    ...
    def slot_fulldepth(self, dummy_sender, data):
        ...
        for order in depth["data"]["asks"]:
            price = price2float(order["price_int"])
            volume = vol2float(order["amount_int"])
    ...
    def log_order(self, price, volume, typ):
        if self.gox.dbcon:
            cur = try_get_cursor(self.gox.dbcon, self.gox.config)
            cur.execute("INSERT INTO Orders(price, volume, type) VALUES(%s, %s, %s)", (price, volume, typ))
            self.gox.dbcon.commit()
            cur.close()

    def calc_total_depth(self):
        
        # Disabled sinced the death of MtGox
        return
        
        if self.ask == 0 or self.bid == 0 or FORCE_NO_FULLDEPTH or self.gox.lastPrice == 0:
            return

        bandpass_a = 0.15
        exp_a = 0.1

        ( bid_usd, bid_usd_linear, bid_usd_exp, bid_usd_bandpass ) = map(int, map(sum, zip(*[( 
                e.price * e.volume, 
                e.price * e.volume * e.price/self.bid, 
                e.price * e.volume * math.exp(-exp_a * ((self.bid - e.price) / self.bid) * 50), 
                e.price * e.volume * (1 - math.exp(-bandpass_a * e.price)), 
            ) for e in self.bids])))

        ( ask_btc, ask_btc_linear, ask_btc_exp, ask_btc_bandpass) = map(int, map(sum, zip(*[(
                e.volume,
                e.volume * max(0, 2-e.price/self.ask),
                e.volume * math.exp(exp_a * ((self.ask - e.price) / self.bid) * 50),
                e.volume * (1 - math.exp(bandpass_a * min(0, (e.price - self.ask - self.bid))))
            ) for e in self.asks])))
        total_ask = ask_btc
        total_bid = bid_usd
        ...
        if self.gox.dbcon:
            cur = try_get_cursor(self.gox.dbcon, self.gox.config)
            cur.execute("INSERT IGNORE INTO OBVol(date, price, bid_usd, ask_btc, bid_usd_bandpass, ask_btc_bandpass, bid_usd_exp, ask_btc_exp, bid_usd_linear, ask_btc_linear) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", 
                (t, self.gox.lastPrice, bid_usd, ask_btc, bid_usd_bandpass, ask_btc_bandpass, bid_usd_exp, ask_btc_exp, bid_usd_linear, ask_btc_linear))
            
            self.gox.dbcon.commit()
            cur.close()
        OBVol = { "key": "OBVol", "values": [[t, bid_usd / ask_btc / self.gox.lastPrice, bid_usd_bandpass / ask_btc_bandpass / self.gox.lastPrice, bid_usd_exp / ask_btc_exp / self.gox.lastPrice, bid_usd_linear / ask_btc_linear / self.gox.lastPrice]] }
        self.gox.send_client_message(OBVol)
